			...
			<thead>
				<tr>
					...
					{if $PS_STOCK_MANAGEMENT}
						...
						<th class="cart_avail item text-left">{l s='Availability'}</th>
					{else}
						{assign var='col_span_subtotal' value='2'}
					{/if}
					<th class="cart_unit item text-left">{l s='Unit price'}</th>
					<th class="cart_quantity item text-left">{l s='Qty'}</th>
					<th class="cart_delete last_item">&nbsp;</th>
					<th class="cart_total item text-left">{l s='Total'}</th>
				</tr>
			</thead>
			<tfoot>
				...
				{if $use_taxes}
					{if $priceDisplay}
						<tr class="cart_total_price">
							...
							<td colspan="{$col_span_subtotal}" class="text-left">{if $display_tax_label}{l s='Total products (tax excl.)'}{else}{l s='Total products'}{/if}</td>
