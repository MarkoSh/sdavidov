...
<ul id="wt-prod-cat-tabs" class="nav nav-tabs clearfix">
	{foreach from=$group_cat_info item=cat_info name=g_cat_info}
		<li class="{if $smarty.foreach.g_cat_info.first} active{/if}"><a data-toggle="tab" href="#wt-prod-cat-{$smarty.foreach.g_cat_info.index}" class="tab_li">
...
<div class="tab-content">
	{foreach from=$group_cat_info item=cat_info name=g_cat_info}
	<div id="wt-prod-cat-{$smarty.foreach.g_cat_info.index}" class="tab-pane {if $smarty.foreach.g_cat_info.first} active{/if}">
		<div class="sub-cat col-sm-2">
			<ul class="sub-cat-ul">
				{foreach from = $cat_info.sub_cat item=sub_cat name=sub_cat_info}
					<li><a href="{$link->getCategoryLink($sub_cat.id_category, $sub_cat.link_rewrite)|escape:'html':'UTF-8'}" title="{$sub_cat.name|escape:'html':'UTF-8'}">{$sub_cat.name}</a></li>
				{/foreach}
				...
				{if isset($cat_info.special_prod_obj) && count($cat_info.special_prod_obj)}
					...
					<li class="wt-prod-special">
						...
						<img class="replace-2x img-responsive" src="{$link->getImageLink($cat_product->link_rewrite[$id_lang], $cat_product->id_image, 'home_default')|escape:'html':'UTF-8'}" alt="" title=""/>
						</a>
					</li>
				{/if}
			</ul>
		...
		<div class="cat-banner col-sm-4">
			{if $cat_info.cat_banner!='' }
				<a href="{$link->getCategoryLink($cat_info.id_cat, $cat_info.link_rewrite)|escape:'html':'UTF-8'}" title="{$cat_info.cat_name|escape:'html':'UTF-8'}">
				<img src="{$banner_path}{$cat_info.cat_banner}" alt=""/>
				</a>
			{/if}
		</div>
		<div class="product_list col-sm-6">
...
$(window).load(function() {
	var owl_pc = $(".owl-prod-cat");
	imagesLoaded(owl_pc, function() {
