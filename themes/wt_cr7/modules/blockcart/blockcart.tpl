...
<div class="out-shopping_cart {if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}
	<div class="shopping_cart">
		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			...
			<span class="ajax_cart_product_txt{if $cart_qties != 1} unvisible{/if}">- {$total}</span>
			<span class="ajax_cart_product_txt_s{if $cart_qties < 2} unvisible{/if}">- {$total}</span>
			...
			<span class="ajax_cart_no_product{if $cart_qties > 0} unvisible{/if}">- {l s='0.00' mod='blockcart'}</span>
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
						...
						<div class="cart-prices">
							<div class="cart-prices-line first-line">
							   <span class="labels_cart" {if !($page_name == 'order-opc') && $shipping_cost_float == 0 && (!isset($cart->id_address_delivery) || !$cart->id_address_delivery)} class="unvisible"{/if}>
							...
							{if $show_tax && isset($tax_cost)}
								<div class="cart-prices-line">
								<span class="labels_cart">{l s='Tax' mod='blockcart'}</span>
									<span class="price cart_block_tax_cost ajax_cart_tax_cost">{$tax_cost}</span>
									
								</div>
							{/if}
							<div class="cart-prices-line out_cart_block_total last-line">
							    <span class="labels_cart">{l s='Total' mod='blockcart'}</span>
...
{if !$PS_CATALOG_MODE && $active_overlay == 1}
	<div id="layer_cart" class="row">
		...
		<div class="layer_cart_cart col-xs-12">
			  <div class="in_layer_cart">
				<h2>
					...
					<span class="ajax_cart_product_txt_s_qv {if $cart_qties < 2} unvisible{/if}">
					...
					<span class="ajax_cart_product_txt_qv {if $cart_qties > 1} unvisible{/if}">
				...
				<div class="button-container">
					<span class="continue btn btn-default button exclusive-medium" title="{l s='Continue shopping' mod='blockcart'}">
						<span>
							<i class="icon-chevron-left left"></i>{l s='Continue' mod='blockcart'}
						</span>
					</span>
		...
		<div class="crossseling col-xs-12"></div>
	</div> <!-- #layer_cart -->
