...
class Blogcomment extends ObjectModel
{
	...
	{
	Shop::addTableAssociation('wt_blog_comment', array('type' => 'shop'));
	...
	public function addComment($id_post, $comment, $value, $id_parent)
	{
		if ($id_parent == '' && $id_parent == null)
			$id_parent = 0;
		$sql = 'INSERT INTO '._DB_PREFIX_.'wt_blog_comment(id_post,name,email,content,website,id_parent, active) VALUES ('.(int)$id_post.', \''.$comment['name'].'\', \''.$comment['mail'].'\', \''.$comment['comment'].'\', \''.$comment['website'].'\', '.$id_parent.', '.$value.')';
	...
	{
	$child_comments = null;
	$sql = 'SELECT * FROM '._DB_PREFIX_.'wt_blog_comment 
	...
	if (isset($child_comments) && (count($child_comments) > 0))
	{
		foreach ($child_comments as $ch_comment)
		{
			...
			{
				$coments_2 = $this->getChildComment($ch_comment['id_wt_blog_comment']);
	...
	public function getComment($id_post)
	{
		...
		{
			$coments = $this->getChildComment($comment['id_wt_blog_comment']);
			if (count($coments) > 0)
	...
	public static function getLatestComments($id_lang = null)
	{
		...
		if (Configuration::get('smartshowhomecomments') != '' && Configuration::get('smartshowhomecomments') != null)
			...
			$limit = 5;
			$sql = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT * FROM '._DB_PREFIX_.'wt_blog_comment bc INNER JOIN 
			'._DB_PREFIX_.'wt_blog_post_shop bps ON bc.id_post=bps.id_wt_blog_post and bps.id_shop = '.(int)Context::getContext()->shop->id.'
			WHERE  bc.active= 1 ORDER BY bc.id_wt_blog_comment DESC limit 0, '.$limit);
			$i = 0;
		foreach ($sql as $post)
		{
			$result[$i]['id_wt_blog_comment'] = $post['id_wt_blog_comment'];
			...
			$result[$i]['slug'] = $WtBlogPost->GetPostSlugById($post['id_post']);
			$i++;
		}
		return $result;
	}
	...
	{
		$sql = 'SELECT id_post FROM '._DB_PREFIX_.'wt_blog_comment 
			WHERE id_post='.$id.' 
			AND active=1';
