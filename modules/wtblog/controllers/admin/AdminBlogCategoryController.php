...
class AdminBlogCategoryController extends AdminController
{
	...
	public function __construct()
	{
		...
		$this->_join = 'LEFT JOIN '._DB_PREFIX_.'wt_blog_category_shop sbs ON a.id_wt_blog_category=sbs.id_wt_blog_category && sbs.id_shop IN('.implode(',', Shop::getContextListShopID()).')';

		$this->_select = 'sbs.id_shop';
		$this->_defaultOrderBy = 'a.id_wt_blog_category';
		$this->_defaultOrderWay = 'DESC';
		
		if (Shop::isFeatureActive() && Shop::getContext() != Shop::CONTEXT_SHOP)
			$this->_group = 'GROUP BY a.id_wt_blog_category';
		parent::__construct();
	}
	...
	public function renderForm()
	{
		...
		if (Tools::getvalue('id_wt_blog_category') != '' && Tools::getvalue('id_wt_blog_category') != null)
			$img_desc .= '<br/><img style="height:auto;width:300px;clear:both;border:1px solid black;" alt="" src="'.__PS_BASE_URI__.'modules/wtblog/views/img/category/'.Tools::getvalue('id_wt_blog_category').'.jpg" /><br />';
	...
	{
		if (Tools::isSubmit('deletewt_blog_category') && Tools::getValue('id_wt_blog_category') != '')
		{
			$id_lang = (int)Context::getContext()->language->id;
			$catpost = (int)WtBlogPost::getToltalByCategory($id_lang, Tools::getValue('id_wt_blog_category'));
			if ((int)$catpost != 0)
				$this->errors[] = Tools::displayError('You need to delete all posts associate with this category .');
			else
			{
				$BlogCategory = new BlogCategory((int)Tools::getValue('id_wt_blog_category'));
		...
		elseif (Tools::isSubmit('submitAddwt_blog_category'))
		{
			parent::validateRules();
			if (count($this->errors))
				return false;
			if (!$id_wt_blog_category = (int)Tools::getValue('id_wt_blog_category'))
			{
			...
			elseif ($id_wt_blog_category = Tools::getValue('id_wt_blog_category'))
			{
				$BlogCategory = new BlogCategory($id_wt_blog_category);
				$languages = Language::getLanguages(false);
				foreach ($languages as $language)
				{
					$title = str_replace('"', '', htmlspecialchars_decode(html_entity_decode(Tools::getValue('meta_title_'.$language['id_lang']))));
				...
				$this->processImageCategory($_FILES, $BlogCategory->id_wt_blog_category);
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminBlogCategory'));
			}
		}
		elseif (Tools::isSubmit('statuswt_blog_category') && Tools::getValue($this->identifier))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				if (Validate::isLoadedObject($object = $this->loadObject()))
				{
					if ($object->toggleStatus())
					{
						...
						$identifier = ((int)$object->id_parent ? '&id_wt_blog_category='.(int)$object->id_parent : '');
						Tools::redirectAdmin($this->context->link->getAdminLink('AdminBlogCategory'));
					}
					else
		...
		elseif (Tools::isSubmit('wt_blog_categoryOrderby') && Tools::isSubmit('wt_blog_categoryOrderway'))
		{
			$this->_defaultOrderBy = Tools::getValue('wt_blog_categoryOrderby');
			$this->_defaultOrderWay = Tools::getValue('wt_blog_categoryOrderway');
		}
	}
	...
	public function processImageCategory($FILES, $id)
	{
		if (isset($FILES['category_image']) && isset($FILES['category_image']['tmp_name']) && !empty($FILES['category_image']['tmp_name']))
		{
			...
			else
			{
				...
				$file_name = $id.'.'.$ext;
				$path = _PS_MODULE_DIR_.'wtblog/views/img/category/'.$file_name;
				...
				else
				{
					...
					{
						$dir = _PS_MODULE_DIR_.'wtblog/views/img/category/'.$id.'-'.Tools::stripslashes($image_type['type_name']).'.jpg';
							if (file_exists($dir))  
								unlink($dir);
					}
					foreach ($images_types as $image_type)
						ImageManager::resize($path, _PS_MODULE_DIR_.'wtblog/views/img/category/'.$id.'-'.Tools::stripslashes($image_type['type_name']).'.jpg', (int)$image_type['width'], (int)$image_type['height']);
				}
			}
		}
	}
