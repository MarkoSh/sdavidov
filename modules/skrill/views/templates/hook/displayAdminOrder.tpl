            ...
            <div id="paymentinfos" class="well col-xs-12">
                <form method='POST' action="{$smarty.server.REQUEST_URI|escape:'htmlall':'UTF-8'}">
                    <div id="paymentinfo">
                        ...
                        {if isset($paymentInfo.orderOrigin)}
                            <div class="form-group">
                                ...
                                <label class="control-label col-lg-9">{$paymentInfo.orderOrigin|escape:'htmlall':'UTF-8'}</label>
                            </div>
                        {/if}
                        {if isset($paymentInfo.orderCountry)}
                            <div class="form-group">
                                ...
                                <label class="control-label col-lg-9">{$paymentInfo.orderCountry|escape:'htmlall':'UTF-8'}</label>
                            </div>
                        {/if}
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label class="col-lg-3">{if {l s='SKRILL_BACKEND_TT_TRANSACTION_ID' mod='skrill'} == "SKRILL_BACKEND_TT_TRANSACTION_ID"} Transaction ID{else}{l s='SKRILL_BACKEND_TT_TRANSACTION_ID' mod='skrill'}{/if}</label>
                            <label class="control-label col-lg-9">{$paymentInfo.transactionId|escape:'htmlall':'UTF-8'}</label>
                        </div>
                        {if isset($paymentInfo.skrillAccount)}
                        <div class="form-group">
                            ...
                            <label class="control-label col-lg-9">{$paymentInfo.skrillAccount|escape:'htmlall':'UTF-8'}</label>
                        </div>
                        {/if}
