...
include_once(_PS_MODULE_DIR_.'wtmegamenu/classes/WtMegamenuClass.php');
include_once(_PS_MODULE_DIR_.'wtmegamenu/classes/WtMegamenuRowClass.php');
include_once(_PS_MODULE_DIR_.'wtmegamenu/classes/WtMegamenuColumnClass.php');
include_once(_PS_MODULE_DIR_.'wtmegamenu/classes/WtMegamenuItemClass.php');
include_once(_PS_MODULE_DIR_.'wtmegamenu/sql/SampleDataMenu.php');

class Wtmegamenu extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('WT Megamenu');
		$this->description = $this->l('Show megamenus');
	...
	public function getContent()
	{
		...
		elseif ((Tools::isSubmit('buildMenu') || Tools::isSubmit('submitRow') || Tools::isSubmit('delete_row')) && Tools::isSubmit('id_wtmegamenu') || Tools::isSubmit('changeStatusRow') || Tools::isSubmit('submitCol') || Tools::isSubmit('delete_col') || Tools::isSubmit('submitSubItem') || Tools::isSubmit('delete_submenu_item') || Tools::isSubmit('changeStatusSubItem'))
		{
			$this->_postProcess();
		...
		elseif (Tools::isSubmit('addMenu') || (Tools::isSubmit('id_wtmegamenu') && $this->menuExists(Tools::getValue('id_wtmegamenu'))))
	...
	public function renderList()
	{
		...
		{
			$info_menus[$key]['status'] = $this->displayStatus($info_menu['id_wtmegamenu'], $info_menu['active']);
	...
	public function getMenuInfo($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT pc.*, pl.*
			FROM '._DB_PREFIX_.'wtmegamenu_shop pc
			LEFT JOIN '._DB_PREFIX_.'wtmegamenu_lang pl ON (pc.id_wtmegamenu = pl.id_wtmegamenu AND pc.`id_shop` = pl.`id_shop`)
			WHERE pl.id_shop = '.$id_shop.' AND pl.id_lang = '.$id_lang.($active ? ' AND pc.`active` = 1' : ' ')
		);
	}
	
	public function displayStatus($id_wtmegamenu, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				...
				&changeStatus&id_wtmegamenu='.(int)$id_wtmegamenu.'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
	...
	public function displayStatusRow($id_menu, $id_row, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				&token='.Tools::getAdminTokenLite('AdminModules').'
				&changeStatusRow&id_wtmegamenu='.$id_menu.'&id_row='.(int)$id_row.'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
	...
	public function displayStatusSubItem($id_menu, $id_item, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				&token='.Tools::getAdminTokenLite('AdminModules').'
				&changeStatusSubItem&id_wtmegamenu='.$id_menu.'&id_item='.(int)$id_item.'" title="'.$title.'"><i class="'.$icon.'"></i></a>';
	...
	protected function _postProcess()
	{
		...
		{
			$this->_clearCache('wtmegamenu.tpl');
			if (Tools::getValue('id_wtmegamenu'))
			{
				$menu_item = new WtMegamenuClass((int)Tools::getValue('id_wtmegamenu'));
				if (!Validate::isLoadedObject($menu_item))
				{
					$this->html .= $this->displayError($this->l('Invalid id_wtmegamenu'));
					return false;
				}
			}
			else
			
				$menu_item = new WtMegamenuClass();
		...
		elseif (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_wtmegamenu'))
		{
			$this->_clearCache('wtmegamenu.tpl');
			$menu = new WtMegamenuClass((int)Tools::getValue('id_wtmegamenu'));
			if ($menu->active == 0)
				$menu->active = 1;
			else
				$menu->active = 0;
		...
		elseif (Tools::isSubmit('removeIcon') && Tools::isSubmit('id_wtmegamenu'))
		{
			$this->_clearCache('wtmegamenu.tpl');
			$menu = new WtMegamenuClass((int)Tools::getValue('id_wtmegamenu'));
			$icon = $menu->icon;
			if (preg_match('/sample/', $icon) === 0)
				if ($icon && file_exists(_PS_MODULE_DIR_.'wtmegamenu/views/img/icons/'.$icon))
					@unlink(_PS_MODULE_DIR_.'wtmegamenu/views/img/icons/'.$icon);
			$menu->icon = '';
			$menu->update();
		}
		elseif (Tools::isSubmit('submitRow'))
		{
			...
			{
				$row_item = new WtMegamenuRowClass(Tools::getValue('id_row'));
				...
				$row_item = new WtMegamenuRowClass();
			$row_item->active = (int)Tools::getValue('active');
			$row_item->id_wtmegamenu = (int)Tools::getValue('id_wtmegamenu');
		...
		{
			$this->_clearCache('wtmegamenu.tpl');
			$menu_item = new WtMegamenuClass((int)Tools::getValue('delete_id_menu'));
			$res = $menu_item->delete();
			if (!$res)
				$this->html .= $this->displayError('Could not delete.');
			else
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_wtmegamenu='.Tools::getValue('id_wtmegamenu'));
		}
		elseif (Tools::isSubmit('delete_row'))
		{
			$this->_clearCache('wtmegamenu.tpl');
			$row_item = new WtMegamenuRowClass((int)Tools::getValue('id_row'));
			$res = $row_item->delete();
			if (!$res)
				$this->html .= $this->displayError('Could not delete.');
			else
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_wtmegamenu='.Tools::getValue('id_wtmegamenu').'&buildMenu=1');
		}
		elseif (Tools::isSubmit('changeStatusRow') && Tools::isSubmit('id_row'))
		{
			...
			$this->_clearCache('wtmegamenu.tpl');
			$id_shop = (int)$this->context->shop->id;
			$row = new WtMegamenuRowClass((int)Tools::getValue('id_row'), null, $id_shop);
			if ($row->active == 0)
				$row->active = 1;
			else
				$row->active = 0;
		...
		elseif (Tools::isSubmit('submitCol'))
		{
			...
			{
				$col_item = new WtMegamenuColumnClass(Tools::getValue('id_column'));
				...
				$col_item = new WtMegamenuColumnClass();
		...
		{
			$this->_clearCache('wtmegamenu.tpl');
			$col_item = new WtMegamenuColumnClass((int)Tools::getValue('id_column'));
			$res = $col_item->delete();
			if (!$res)
				$this->html .= $this->displayError('Could not delete.');
			else
		...
		elseif (Tools::isSubmit('submitSubItem'))
		{
			...
			{
				$sub_menu_item = new WtMegamenuItemClass(Tools::getValue('id_item'));
				...
				$sub_menu_item = new WtMegamenuItemClass();
		...
		{
			$this->_clearCache('wtmegamenu.tpl');
			$submenu_item = new WtMegamenuItemClass((int)Tools::getValue('id_item'));
	...
	public function renderAddForm()
	{
		...
		$id_wtmegamenu = Tools::getValue('id_wtmegamenu');
		if (isset($id_wtmegamenu))
			$menu = new WtMegamenuClass($id_wtmegamenu);
		else
			$menu = new WtMegamenuClass();
	...
	public function renderAddMeniItem()
	{
		...
		$fields_form = array(
			'form' => array(
				...
				'buttons' => array(
					array(
					'href' => AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'&id_wtmegamenu='.$id_wtmegamenu.'&buildMenu=1',
					'title' => $this->l('Cancel'),
		...
		if (Tools::isSubmit('id_item'))
		{
			...
			$subMenu_item = new WtMegamenuItemClass(Tools::getValue('id_item'));
		...
		$helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_wtmegamenu='.$id_wtmegamenu.'&buildMenu=1';
	...
	public function getAddFieldsValuesLink()
	{
		...
		if (Tools::isSubmit('id_item'))
		{
			$menu_item = new WtMegamenuItemClass((int)Tools::getValue('id_item'));
		...
		{
			$menu_item = new WtMegamenuItemClass();
	...
	public function renderAddRow()
	{
		...
		$fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_wtmegamenu');
	...
	public function getAddFieldsValues()
	{
		...
		if (Tools::isSubmit('id_row'))
		{
			$row = new WtMegamenuRowClass((int)Tools::getValue('id_row'));
			$fields['id_row'] = (int)Tools::getValue('id_row', $row->id);
			$fields['active'] = Tools::getValue('active', $row->active);
			$fields['id_wtmegamenu'] = Tools::getValue('id_wtmegamenu', $row->id_wtmegamenu);
			$fields['class'] = Tools::getValue('class', $row->class);
		}
		else
		{
			$row = new WtMegamenuRowClass();
			$fields['active'] = Tools::getValue('active', 1);
			$fields['id_wtmegamenu'] = Tools::getValue('id_wtmegamenu', 1);
	...
	public function getAddFieldsValuesCol()
	{
		...
		if (Tools::isSubmit('id_column'))
		{
			$col = new WtMegamenuColumnClass((int)Tools::getValue('id_column'));
	...
	public function renderBuildMenu()
	{
		$this->context->controller->addJS($this->_path.'views/js/back.js');
		$this->context->controller->addCSS($this->_path.'views/css/back.css');
		$id_wtmegamenu = Tools::getValue('id_wtmegamenu', 1);
		$info_rows = $this->getRowInfo($id_wtmegamenu);
		
		foreach ($info_rows as $key => $info_row)
		{
			$info_rows[$key]['status'] = $this->displayStatusRow($id_wtmegamenu, $info_row['id_row'], $info_row['active']);
			$info_rows[$key]['list_col'] = $this->getColInfo($info_row['id_row']);
		}
		$this->context->smarty->assign(
			array(
				'id_wtmegamenu' => $id_wtmegamenu,
				'token' => Tools::getAdminTokenLite('AdminModules'),
				'info_rows' => $info_rows,
				'link' => $this->context->link
			)
	...
	public function getRowInfo($id_menu, $active = null)
	{
		...
		$row_infos_rs = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mr.*
			FROM '._DB_PREFIX_.'wtmegamenu_row_shop mr
			WHERE mr.`id_shop` = '.$id_shop.' AND mr.id_wtmegamenu = '.$id_menu.($active ? ' AND mr.`active` = 1' : ' ')
		);
	...
	public function getColInfo($id_row, $active = null, $is_backend = true)
	{
		...
		$cols_result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mc.*
			FROM '._DB_PREFIX_.'wtmegamenu_column_shop mc
		...
		if (is_array($col_infos) && count($col_infos) > 0)
			foreach ($col_infos as $key => $col_info)
			{
				$sub_menu_infos = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
					SELECT smi.*,smil.*
					FROM '._DB_PREFIX_.'wtmegamenu_item_shop smi
					LEFT JOIN '._DB_PREFIX_.'wtmegamenu_item_lang smil ON (smi.id_item = smil.id_item AND smi.id_shop = smil.id_shop)
					WHERE smi.id_column = '.$col_info['id_column'].' AND smil.id_shop = '.$id_shop.' AND smil.id_lang = '.$id_lang.($active ? ' AND smi.`active` = 1' : ''));
				...
				if (is_array($sub_menu_infos) && count($sub_menu_infos) > 0)
				{
					...
					{
						$id_wtmegamenu = Tools::getValue('id_wtmegamenu', 1);
						if (isset($id_wtmegamenu) && $id_wtmegamenu > 0 && $is_backend)
							$sub_menu_infos[$key1]['status'] = $this->displayStatusSubItem($id_wtmegamenu, $sub_menu_info['id_item'], $sub_menu_info['active']);
	...
	public function getSubMenu($id_wtmegamenu)
	{
		$info_rows = array();
		if (is_array($this->getRowInfo($id_wtmegamenu, true)) && count($this->getRowInfo($id_wtmegamenu, true)) > 0)
		{
			$info_rows = $this->getRowInfo($id_wtmegamenu, true);
			foreach ($info_rows as $key => $info_row)
	...
	{
		$req = 'SELECT wt.`id_wtmegamenu` as id_wtmegamenu
				FROM `'._DB_PREFIX_.'wtmegamenu` wt
				WHERE wt.`id_wtmegamenu` = '.(int)$id;
	...
	public function hookDisplayTop()
	{
		...
		return $this->display(__FILE__, 'wtmegamenu.tpl', $this->getCacheId('wtmegamenu'));
	}
	...
	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'wtmegamenu_shop (`id_wtmegamenu`, `id_shop`, `type_link`, `dropdown`, `type_icon`, `icon`, `align_sub`, `width_sub`, `class`, `active`)
			SELECT id_wtmegamenu, '.(int)$params['new_id_shop'].', type_link, dropdown, type_icon, icon, align_sub, width_sub, class, active
			FROM '._DB_PREFIX_.'wtmegamenu_shop
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'wtmegamenu_lang (`id_wtmegamenu`, `id_shop`, `id_lang`, `title`, `link`, `subtitle`)
			SELECT id_wtmegamenu, '.(int)$params['new_id_shop'].', id_lang, title, link, subtitle
			FROM '._DB_PREFIX_.'wtmegamenu_lang
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'wtmegamenu_row_shop (`id_row`, `id_wtmegamenu`, `id_shop`, `class`, `active`)
			SELECT id_row, id_wtmegamenu, '.(int)$params['new_id_shop'].', class, active
			FROM '._DB_PREFIX_.'wtmegamenu_row_shop
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'wtmegamenu_column_shop (`id_column`, `id_row`, `id_shop`, `width`, `class`, `active`)
			SELECT id_column, id_row, '.(int)$params['new_id_shop'].', width, class, active
			FROM '._DB_PREFIX_.'wtmegamenu_column_shop
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'wtmegamenu_item_shop (`id_item`, `id_column`, `id_shop`, `type_link`, `type_item`, `id_product`, `active`)
			SELECT id_item, id_column, '.(int)$params['new_id_shop'].', type_link, type_item, id_product, active
			FROM '._DB_PREFIX_.'wtmegamenu_item_shop
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
		
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'wtmegamenu_item_lang (`id_item`, `id_shop`, `id_lang`, `title`, `link`, `text`)
			SELECT id_item, '.(int)$params['new_id_shop'].', id_lang, title, link, text
			FROM '._DB_PREFIX_.'wtmegamenu_item_lang
			WHERE id_shop = '.(int)$params['old_id_shop']
		);
	}
	public function hookActionObjectLanguageAddAfter($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'wtmegamenu_lang (`id_wtmegamenu`, `id_shop`, `id_lang`, `title`, `link`, `subtitle`)
			SELECT id_wtmegamenu, id_shop, '.(int)$params['object']->id.', title, link, subtitle
			FROM '._DB_PREFIX_.'wtmegamenu_lang
