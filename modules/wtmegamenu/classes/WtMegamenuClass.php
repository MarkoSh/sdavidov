...
class WtMegamenuClass extends ObjectModel
{
	public $type_link;
	public $dropdown;
	public $type_icon;
	...
	public	function __construct($id_wtmegamenu = null, $id_lang = null, $id_shop = null, Context $context = null)
	{
		parent::__construct($id_wtmegamenu, $id_lang, $id_shop);
		Shop::addTableAssociation('wtmegamenu', array('type' => 'shop'));
		Shop::addTableAssociation('wtmegamenu_lang', array('type' => 'fk_shop'));
		if ($this->id)
	...
	public function getFieldShop($field)
	{
		...
		$sql = 'SELECT wms.'.$field.' FROM '._DB_PREFIX_.'wtmegamenu wm
		LEFT JOIN '._DB_PREFIX_.'wtmegamenu_shop wms ON (wm.id_wtmegamenu = wms.id_wtmegamenu)
		WHERE wm.id_wtmegamenu = '.$this->id.' AND wms.id_shop = '.$id_shop.'';
	...
	public function delete()
	{
		...
		if (preg_match('/sample/', $icon) === 0)
			if ($icon && file_exists(_PS_MODULE_DIR_.'wtmegamenu/views/img/icons/'.$icon))
				$res &= @unlink(_PS_MODULE_DIR_.'wtmegamenu/views/img/icons/'.$icon);
	...
	public function getColumByRow($id_row)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mc.*
			FROM '._DB_PREFIX_.'wtmegamenu_column mc
	...
	public function deleteMenuItem($id_col)
	{
		...
		$menu_items = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mi.*
			FROM '._DB_PREFIX_.'wtmegamenu_item mi
		...
		if (isset ($menu_items) && count($menu_items) > 0)
		{
			foreach ($menu_items as $menu_item)
			{
				$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtmegamenu_item_lang`
					WHERE `id_item` = '.$menu_item['id_item']
				);
				
				$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtmegamenu_item_shop`
					WHERE `id_item` = '.$menu_item['id_item']
				);
				
				$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtmegamenu_item`
					WHERE `id_item` = '.$menu_item['id_item']
				);
			}
		}
	...
	public function deleteColumnItem($id_col)
	{
		$res = true;
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'wtmegamenu_column_shop`
			WHERE `id_column` = '.$id_col
		);
		$res &= Db::getInstance()->execute('
				DELETE FROM `'._DB_PREFIX_.'wtmegamenu_column`
				WHERE `id_column` = '.$id_col
			);
		return $res;
	}
	...
	public function deleteRowItem($id_row)
	{
		$res = true;
		$res &= Db::getInstance()->execute('
				DELETE FROM `'._DB_PREFIX_.'wtmegamenu_row_shop`
				WHERE `id_row` = '.$id_row
			);
		
		$res &= Db::getInstance()->execute('
				DELETE FROM `'._DB_PREFIX_.'wtmegamenu_row`
				WHERE `id_row` = '.$id_row
			);
		return $res;
	}
	...
	public function getRowByMenu($id_menu)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mr.*
			FROM '._DB_PREFIX_.'wtmegamenu_row mr
			WHERE mr.`id_wtmegamenu` = '.$id_menu);
	}
	...
	public function getMenus()
	{
		...
		$kq = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT wt.*, wl.*
			FROM '._DB_PREFIX_.'wtmegamenu_shop wt
			LEFT JOIN `'._DB_PREFIX_.'wtmegamenu_lang` wl ON (wl.`id_wtmegamenu` = wt.`id_wtmegamenu` AND wt.`id_shop` = wl.`id_shop`)
