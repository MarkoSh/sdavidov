...
class AdminAddThumbnailImageController extends ModuleAdminController
{
	...
	{
		parent::setMedia();
		$this->addJS(_MODULE_DIR_.'wtslideshow/views/js/admin/uploadimage_thumb.js');
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitUploadImage'))
		{
			$type = Tools::strtolower(Tools::substr(strrchr($_FILES['upload_image']['name'], '.'), 1));
				...
				$width_img = (isset($imagesize[0]) && $imagesize[0])? (int)$imagesize[0] : null;
				$height_img = (isset($imagesize[1]) && $imagesize[1])? (int)$imagesize[1] : null;
				if ($width_img > 400)
				{
					$width_img = 400;
					$height_img = (int)($width_img * $imagesize[1] / $imagesize[0]);
				}
					...
					) && in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
				{
					...
					elseif (!$temp_name || !move_uploaded_file($_FILES['upload_image']['tmp_name'], $temp_name))
						return false;
					elseif (!ImageManager::resize($temp_name, _PS_MODULE_DIR_.'wtslideshow/views/img/thumbnails/'.$salt.'_'.$_FILES['upload_image']['name'], $width_img, $height_img, $type))
	...
	public function display()
	{
		...
		$action_url = $this->context->link->getAdminLink('AdminAddThumbnailImage');
		...
		$this->context->smarty->assign(
			...
			'id_lang' => $id_lang,
			'image_path'  => _MODULE_DIR_.'wtslideshow/views/img/thumbnails/',
			'cs_ajax_link' => _PS_BASE_URL_SSL_.__PS_BASE_URI__.'modules/wtslideshow/ajax_wtslideshow.php'
		));
		$this->smartyOutputContent(_PS_MODULE_DIR_.'wtslideshow/views/templates/admin/thumbnailform.tpl');
		return true;
	}
	public function readImages()
	{
		$list_images = array();
		$handle = opendir(_PS_MODULE_DIR_.'wtslideshow/views/img/thumbnails');
		if ($handle)
		{
			while (false !== ($entry = readdir($handle)))
			{
				if ($entry != '.' && $entry != '..')
				{
					if (file_exists(_PS_MODULE_DIR_.'wtslideshow/views/img/thumbnails/'.$entry))
