...
class AdminAddLayerImageController extends ModuleAdminController
{
	...
	{
		parent::setMedia();
		$this->addJS(_MODULE_DIR_.'wtslideshow/views/js/admin/uploadimage.js');
			...
			$type = Tools::strtolower(Tools::substr(strrchr($_FILES['upload_image']['name'], '.'), 1));
					...
					) && in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
				{
					...
					elseif (!$temp_name || !move_uploaded_file($_FILES['upload_image']['tmp_name'], $temp_name))
						return false;
					elseif (!ImageManager::resize($temp_name, _PS_MODULE_DIR_.'wtslideshow/views/img/layers/'.$salt.'_'.$_FILES['upload_image']['name'], null, null, $type))
	...
	public function display()
	{
		...
		$action_url = $this->context->link->getAdminLink('AdminAddLayerImage');
		...
		$this->context->smarty->assign(
			...
			'id_lang' => $id_lang,
			'image_path'  => _MODULE_DIR_.'wtslideshow/views/img/layers/',
			'cs_ajax_link' => _PS_BASE_URL_SSL_.__PS_BASE_URI__.'modules/wtslideshow/ajax_wtslideshow.php'
		));
		$this->smartyOutputContent(_PS_MODULE_DIR_.'wtslideshow/views/templates/admin/layermedia.tpl');
		return true;
	}
	public function readImages()
	{
		$list_images = array();
		$handle = opendir(_PS_MODULE_DIR_.'wtslideshow/views/img/layers');
		if ($handle)
		{
			while (false !== ($entry = readdir($handle)))
			{
				if ($entry != '.' && $entry != '..')
				{
					if (file_exists(_PS_MODULE_DIR_.'wtslideshow/views/img/layers/'.$entry))
						if (strpos($entry, 'index') === false && strpos($entry, 'Thumbs') === false)
							$list_images[] = $entry;
				}
			}
			closedir($handle);	
