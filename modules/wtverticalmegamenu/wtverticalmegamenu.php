...
class WtVerticalMegamenu extends Module
{
	...
	public function __construct()
	{
		...
		$this->bootstrap = true;
		parent::__construct();
		$this->displayName = $this->l('WT Vertical Megamenu');
	...
	public function displayStatus($id_wtverticalmegamenu, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				...
				&changeStatus&id_wtverticalmegamenu='.(int)$id_wtverticalmegamenu.'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
		return $html;
	}
	...
	public function displayStatusRow($id_menu, $id_row, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				&token='.Tools::getAdminTokenLite('AdminModules').'
				&changeStatusRow&id_wtverticalmegamenu='.$id_menu.'&id_row='.(int)$id_row.'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
		return $html;
	}
	...
	public function displayStatusSubItem($id_menu, $id_item, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				&token='.Tools::getAdminTokenLite('AdminModules').'
				&changeStatusSubItem&id_wtverticalmegamenu='.$id_menu.'&id_item='.(int)$id_item.'" title="'.$title.'"><i class="'.$icon.'"></i></a>';
		return $html;
	}
		...
		if (is_array($col_infos) && count($col_infos) > 0)
			foreach ($col_infos as $key => $col_info)
			{
				$sub_menu_infos = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
					...
					WHERE smil.`id_shop` = '.$id_shop.' AND smi.id_column = '.$col_info['id_column'].' AND smil.id_lang = '.$id_lang.($active ? ' AND smi.`active` = 1' : ''));
	...
	public function getSubMenu($id_wtverticalmegamenu)
	{
		$info_rows = array();
		if (is_array($this->getRowInfo($id_wtverticalmegamenu)) && count($this->getRowInfo($id_wtverticalmegamenu)) > 0)
		{
			$info_rows = $this->getRowInfo($id_wtverticalmegamenu);
			foreach ($info_rows as $key => $info_row)
				$info_rows[$key]['list_col'] = $this->getColInfo($info_row['id_row'], null, false);
		}
		return $info_rows;
	}
	...
	public function prevHook()
	{
		...
		foreach ($menus as $menu)
		{
			...
			else
			{
				...
				$sub_menu = $this->getSubMenu($menu['id_wtverticalmegamenu']);
	...
	{
		if (!$this->isCached('wtverticalmegamenu_page.tpl', $this->getCacheId('wtverticalmegamenu_page')))
			$this->prevHook();
		return $this->display(__FILE__, 'wtverticalmegamenu_page.tpl', $this->getCacheId('wtverticalmegamenu_page'));
