...
class WtVerticalMegamenuItemClass extends ObjectModel
{
	public $id_column;
	public $type_link;
	public $type_item;
	...
	public $text;
	public $active;
	public $temp_url = '{wt_menu_v_url}';
	public static $definition = array(
		'table' => 'wtverticalmegamenu_item',
		'primary' => 'id_item',
		'multilang' => true,
		'multilang_shop' => true,
		'fields' => array(
			'id_column' =>	array('type' => self::TYPE_BOOL, 'shop' => true, 'validate' => 'isunsignedInt', 'required' => true),
			'type_link' =>	array('type' => self::TYPE_BOOL, 'shop' => true, 'validate' => 'isunsignedInt', 'required' => true, 'size' => 255),
			'type_item' =>	array('type' => self::TYPE_BOOL, 'shop' => true, 'validate' => 'isunsignedInt', 'required' => true, 'size' => 255),
			'id_product' =>	array('type' => self::TYPE_BOOL, 'shop' => true, 'validate' => 'isunsignedInt', 'size' => 255),
	...
	public	function __construct($id_item = null, $id_lang = null, $id_shop = null, Context $context = null)
	{
		...
		Shop::addTableAssociation('wtverticalmegamenu_item', array('type' => 'shop'));
		Shop::addTableAssociation('wtverticalmegamenu_item_lang', array('type' => 'fk_shop'));
		if ($this->id)
		{
			...
			{
				if (isset($this->text[(int)($language['id_lang'])]) && !empty($this->text[(int)($language['id_lang'])]))
				{
					$temp = str_replace($this->temp_url, _PS_BASE_URL_.__PS_BASE_URI__, $this->text[(int)($language['id_lang'])]);
	...
	public function getFieldShop($field)
	{
		...
		$sql = 'SELECT wms.'.$field.' FROM '._DB_PREFIX_.'wtverticalmegamenu_item wm
		LEFT JOIN '._DB_PREFIX_.'wtverticalmegamenu_item_shop wms ON (wm.id_item = wms.id_item)
