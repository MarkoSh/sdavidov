...
class WtVerticalMegamenuClass extends ObjectModel
{
	public $type_link;
	public $dropdown;
	public $type_icon;
	...
	public static $definition = array(
		'table' => 'wtverticalmegamenu',
		'primary' => 'id_wtverticalmegamenu',
		'multilang' => true,
		'multilang_shop' => true,
		'fields' => array(
			'type_link' =>	array('type' => self::TYPE_BOOL, 'shop' => true, 'validate' => 'isunsignedInt', 'required' => true, 'size' => 255),
			'dropdown' =>	array('type' => self::TYPE_BOOL, 'shop' => true, 'validate' => 'isunsignedInt', 'required' => true),
			'type_icon' =>	array('type' => self::TYPE_BOOL, 'shop' => true, 'validate' => 'isunsignedInt', 'required' => true),
	...
	public	function __construct($id_wtverticalmegamenu = null, $id_lang = null, $id_shop = null, Context $context = null)
	{
		parent::__construct($id_wtverticalmegamenu, $id_lang, $id_shop);
		Shop::addTableAssociation('wtverticalmegamenu', array('type' => 'shop'));
		Shop::addTableAssociation('wtverticalmegamenu_lang', array('type' => 'fk_shop'));
		if ($this->id)
	...
	public function getFieldShop($field)
	{
		...
		$sql = 'SELECT wms.'.$field.' FROM '._DB_PREFIX_.'wtverticalmegamenu wm
		LEFT JOIN '._DB_PREFIX_.'wtverticalmegamenu_shop wms ON (wm.id_wtverticalmegamenu = wms.id_wtverticalmegamenu)
		WHERE wm.id_wtverticalmegamenu = '.$this->id.' AND wms.id_shop = '.$id_shop.'';
	...
	public function delete()
	{
		...
		if (preg_match('/sample/', $icon) === 0)
			if ($icon && file_exists(_PS_MODULE_DIR_.'wtverticalmegamenu/views/img/icons/'.$icon))
				$res &= @unlink(_PS_MODULE_DIR_.'wtverticalmegamenu/views/img/icons/'.$icon);
	...
	public function getColumByRow($id_row)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mc.*
			FROM '._DB_PREFIX_.'wtverticalmegamenu_column mc
	...
	public function deleteMenuItem($id_col)
	{
		...
		$menu_items = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mi.*
			FROM '._DB_PREFIX_.'wtverticalmegamenu_item mi
		...
		if (isset ($menu_items) && count($menu_items) > 0)
		{
			foreach ($menu_items as $menu_item)
			{
				$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtverticalmegamenu_item_lang`
					WHERE `id_item` = '.$menu_item['id_item']
				);
				
				$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtverticalmegamenu_item_shop`
					WHERE `id_item` = '.$menu_item['id_item']
				);
				
				$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtverticalmegamenu_item`
					WHERE `id_item` = '.$menu_item['id_item']
				);
			}
		}
	...
	public function deleteColumnItem($id_col)
	{
		$res = true;
		$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtverticalmegamenu_column_shop`
					WHERE `id_column` = '.$id_col
				);
		$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtverticalmegamenu_column`
					WHERE `id_column` = '.$id_col
				);
		return $res;
	}
	...
	public function deleteRowItem($id_row)
	{
		$res = true;
		$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtverticalmegamenu_row_shop`
					WHERE `id_row` = '.$id_row
				);
		$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'wtverticalmegamenu_row`
					WHERE `id_row` = '.$id_row
				);
		return $res;
	}
	...
	public function getRowByMenu($id_menu)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mr.*
			FROM '._DB_PREFIX_.'wtverticalmegamenu_row mr
			WHERE mr.`id_wtverticalmegamenu` = '.$id_menu);
	}
	...
	public function getMenus()
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT wt.*, wl.*
			FROM '._DB_PREFIX_.'wtverticalmegamenu_shop wt
			LEFT JOIN `'._DB_PREFIX_.'wtverticalmegamenu_lang` wl ON (wl.`id_wtverticalmegamenu` = wt.`id_wtverticalmegamenu` AND wt.`id_shop` = wl.`id_shop`)
			WHERE wt.active = 1 AND wl.id_shop = '.$id_shop.' AND wl.id_lang='.$id_lang);
	}
