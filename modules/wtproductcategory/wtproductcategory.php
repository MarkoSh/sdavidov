...
class Wtproductcategory extends Module
{
	...
	private $html = '';
	private $hook_into = array('displayHome', 'displayTopColumn', 'displayLeftColumn','displayRightColumn','displayTopHome', 'displayBottomHome');
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('WT Product Category');
	...
	public function displayStatus($id_wtgroupcategory, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				...
				&changeStatus&id_wtgroupcategory='.(int)$id_wtgroupcategory.'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
	...
	public function displayStatusCatItem($id_wtgroupcategory, $id_wtcategory, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				&token='.Tools::getAdminTokenLite('AdminModules').'
				&changeStatusCatItem&id_wtgroupcategory='.(int)$id_wtgroupcategory.'&id_wtcategory='.(int)$id_wtcategory.'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
	...
	protected function _postProcess()
	{
		...
		elseif (Tools::isSubmit('submitCatItem'))
		{
			...
			{
				$cat_item->cat_desc[$language['id_lang']] = Tools::getValue('cat_desc_'.$language['id_lang']);
	...
	public function prevHook($hook_name)
	{
		...
		foreach ($group_cat_hook as $group_cat)
		{
			...
			foreach ($cat_prod_group as $cat_prod)
			{
				...
				$product_list = $category->getProducts($id_lang, 1, $number_prod, 'position');
	...
	{
		if (!$this->isCached('wtproductcategory_home.tpl', $this->getCacheId('wtproductcategory_home')))
		{
		...
		return $this->display(__FILE__, 'wtproductcategory_home.tpl', $this->getCacheId('wtproductcategory_home'));
	}
	
	public function hookdisplayTopColumn()
	{
		if (!$this->isCached('wtproductcategory_topcolumn.tpl', $this->getCacheId('wtproductcategory_topcolumn')))
		{
		...
		return $this->display(__FILE__, 'wtproductcategory_topcolumn.tpl', $this->getCacheId('wtproductcategory_topcolumn'));
	}
	
	public function hookdisplayLeftColumn()
	{
		if (!$this->isCached('wtproductcategory_leftcolumn.tpl', $this->getCacheId('wtproductcategory_leftcolumn')))
		{
			$group_cat_result = $this->prevHook('displayLeftColumn');
			if (!isset($group_cat_result) || count($group_cat_result) <= 0)
				return false;
			$this->context->smarty->assign(array(
				'group_cat_result' => $group_cat_result,
		...
		return $this->display(__FILE__, 'wtproductcategory_leftcolumn.tpl', $this->getCacheId('wtproductcategory_leftcolumn'));
	}
	
	public function hookdisplayRightColumn()
	{
		if (!$this->isCached('wtproductcategory_rightcolumn.tpl', $this->getCacheId('wtproductcategory_rightcolumn')))
		{
			$group_cat_result = $this->prevHook('displayRightColumn');
			if (!isset($group_cat_result) || count($group_cat_result) <= 0)
				return false;
			$this->context->smarty->assign(array(
				'group_cat_result' => $group_cat_result,
		...
		return $this->display(__FILE__, 'wtproductcategory_rightcolumn.tpl', $this->getCacheId('wtproductcategory_rightcolumn'));
	}
	
	public function hookdisplayTopHome()
	{
		if (!$this->isCached('wtproductcategory_tophome.tpl', $this->getCacheId('wtproductcategory_tophome')))
		{
		...
		return $this->display(__FILE__, 'wtproductcategory_tophome.tpl', $this->getCacheId('wtproductcategory_tophome'));
	}
	
	public function hookdisplayBottomHome()
	{
		if (!$this->isCached('wtproductcategory_bottomhome.tpl', $this->getCacheId('wtproductcategory_bottomhome')))
		{
		...
		return $this->display(__FILE__, 'wtproductcategory_bottomhome.tpl', $this->getCacheId('wtproductcategory_bottomhome'));
	}
	...
	public function clearCacheProdCat()
	{
		...
		$this->_clearCache('wtproductcategory_rightcolumn.tpl');
		$this->_clearCache('wtproductcategory_leftcolumn.tpl');
